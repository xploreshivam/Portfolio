...
// Project cards Three-dimensional tilt & hover
...
function showToast(msg) {
  ...
  toast.style.cssText = `
    ...
    background: rgba(11, 12, 15, 0.95); border: 1px solid var(--accent, #6ab3ff);
    color: #fff; padding: 12px 24px; border-radius: 8px;
    z-index: 10001; animation: fadeInUp 0.3s ease; font-size: 0.9rem;
...
// Global click listener for placeholder links
document.addEventListener('click', (e) => {
  ...
  if (href === '#' || href === '') {
    if (
      link.closest('.notebook-vault-card') ||
      link.closest('.case-studies-card') ||
      link.id === 'open-notebook-btn-card' ||
      link.id === 'open-case-studies-btn-card'
    ) {
      return;
    }
    e.preventDefault();
    ...
    const isRead = text.includes('read') || text.includes('case');
    let msg = 'Live Demo coming soon!';
    if (isRead) {
      msg = 'Detailed case study dossier is currently private.';
    } else if (!isLive) {
      msg = 'Repository is currently private.';
    }
    showToast(msg);
  }
});
...
// SPLIT-SCREEN NAVIGATION SYSTEM
(function initSplitScreenNav() {
  ...
  function openSectionInSplit(targetId) {
    ...
    clonedContainer.querySelectorAll('[id]').forEach(el => {
      ...
      el.setAttribute('id', 'panel-' + oldId);
    });
    clonedContainer.querySelectorAll('[aria-describedby]').forEach(el => {
      const oldVal = el.getAttribute('aria-describedby');
      if (oldVal) el.setAttribute('aria-describedby', 'panel-' + oldVal);
    });
    clonedContainer.querySelectorAll('[for]').forEach(el => {
      const oldVal = el.getAttribute('for');
      if (oldVal) el.setAttribute('for', 'panel-' + oldVal);
    });
    ...
    if (openVaultCard) {
      ...
      openVaultCard.addEventListener('keydown', (e) => {
    ...
    if (openCaseCard) {
      ...
      openCaseCard.addEventListener('keydown', (e) => {
        if ((e.key === 'Enter' || e.key === ' ') && !e.target.closest('a')) {
          e.preventDefault();
          e.stopPropagation();
          triggerCase();
...
// CONTACT FORM SUBMISSION ENGINE (EMAILJS INTEGRATION)
// EmailJS Setup Credentials
// 1. Sign up / Log in at https://www.emailjs.com/

// Create an Email Template -> Copy Template ID below
// Go to Account > API Keys -> Copy Public Key (User ID) below
const EMAILJS_PUBLIC_KEY = "sL8lyoAzaeyc3CpDm";
const SERVICE_ID = "service_jaswmq8";
const TEMPLATE_ID = "template_pvxfu89";
...
async function handleContactFormSubmit(targetForm) {
  ...
  if (nameInput) nameInput.setAttribute('aria-invalid', !name ? 'true' : 'false');
  if (emailInput) emailInput.setAttribute('aria-invalid', !email ? 'true' : 'false');
  if (messageInput) messageInput.setAttribute('aria-invalid', !message ? 'true' : 'false');
  ...
  try {
    ...
    // Fallback: Open mailto link safely so message is never lost
...
// DEVELOPER'S MINI PROJECTS VAULT SYSTEM
(function initMiniProjectsVault() {
  ...
  function openNotebook() {
    ...
    if (notebookModal) {
      ...
      document.body.style.overflow = 'hidden';
      if (closeNotebookBtn) closeNotebookBtn.focus();
    }
  ...
  if (openCardContainer) {
    ...
    openCardContainer.addEventListener('keydown', (e) => {
      if ((e.key === 'Enter' || e.key === ' ') && !e.target.closest('a')) {
        e.preventDefault();
        openNotebook();
      }
  ...
  function renderGallery() {
    ...
    if (items.length === 0) {
      const displayTerm = escapeHTML(searchQuery || activeFilter);
      galleryGrid.innerHTML = `
        <div style="grid-column:1/-1; text-align:center; padding:40px 20px; color:var(--muted); width:100%;">
          <p>No mini projects found matching "<strong>${displayTerm}</strong>".</p>
        </div>
      `;
      return;
    }
...
// TECHNICAL CASE STUDIES VAULT SYSTEM
(function initCaseStudiesModal() {
  ...
  function openCaseModal() {
    ...
    if (caseModal) {
      ...
      document.body.style.overflow = 'hidden';
      if (closeCaseBtn) closeCaseBtn.focus();
    }
  ...
  if (openCaseContainer) {
    ...
    openCaseContainer.addEventListener('keydown', (e) => {
      if ((e.key === 'Enter' || e.key === ' ') && !e.target.closest('a')) {
        e.preventDefault();
        openCaseModal();
      }
  ...
  function renderCaseStudies() {
    ...
    if (filtered.length === 0) {
      const displayTerm = escapeHTML(csSearchQuery || activeCsFilter);
      caseGalleryGrid.innerHTML = `
        <div class="gallery-empty-state" style="grid-column: 1 / -1; text-align:center; padding:40px; color:var(--muted);">
          <p>No case studies found matching "${displayTerm}".</p>
        </div>
      `;
      return;
    }
